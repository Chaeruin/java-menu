...
- [ ] 입력
    - [ ] 코치 이름 입력
        - [ ] 코치 이름이 각각 2자에서 4자 사이(공백제외trim)가 아닌 경우 예외
        - [ ] 코치가 2명 미만 5명 초과일 경우 예외
        - [ ] 코치 이름에 빈문자가 들어갈 경우 예외
        - [ ] 구분자(,)가 제대로 되어있지 않을 경우 예외
    - [ ] 각 코치마다 못먹는 메뉴 입력
        - [ ] 구분자(,)가 제대로 되어있지 않을 경우 예외
        - [ ] 입력이 2개를 초과할 경우 예외
        - [ ] 구분자 사이에 빈 값이 있을 경우(그냥 빈값 말고) 는 예외
- [ ] 출력
    - [ ] 점심 메뉴 추천 시작
    - [ ] 메뉴 추천 결과 (구분(요일/카테고리 선택된거/이름과 메뉴 선택된거)
    - [ ] 추천을 완료했습니다 -> 종료
- [ ] 기능
    - [ ] 카테고리-메뉴 초기화 입력
    - [ ] 카테고리 랜덤 뽑기 (Randoms.pickNumberInRange(1,5);)
        - [ ] 1이면 일식, 2면 한식, 3이면 중식, 4면 아시안, 5면 양식
        - [ ] 만약 2번 초과로 같은 값이 나왔으면 다시 임의 랜덤 값을 생성해서 추천 카테고리 정하기
    - [ ] 추천할 카테고리에서 메뉴 뽑기
        - [ ] 만약 제외해야 할 메뉴가 해당 카테고리인 경우 제외하고 뽑기
        - [ ] 뽑는 로직은 Randoms.shuffle(menus).get(0) -> 셔플하고 첫번째 요소 가져오는 방식으로
